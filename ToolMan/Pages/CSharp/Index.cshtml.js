$(function () {
    ...
    $('#GenerateLButton').click(function (e) {
        ...
        localStorage.setItem("abp_outPutPath", $("#ViewModel_OutputPath").val());
    })
    ...
    if (localStorage.getItem("abp_outPutPath")) {
        $("#ViewModel_OutputPath").val(localStorage.getItem("abp_outPutPath"));
    }
    ...
    function Generate(path) {
        ...
        if (input.outputPath == '' || input.outputPath.indexOf('\\') == 0) {
    ...
    $("#ViewModel_TemplatePath").keydown(function (event) {
        if (event.keyCode === 13) {
            ...
            localStorage.setItem("abp_templatePath", event.currentTarget.value);
            location.reload();
        }
    });

    if (localStorage.getItem("abp_templatePath")) {
        $("#ViewModel_TemplatePath").val(localStorage.getItem("abp_templatePath"));
        LoadTree();
    }
    ...
    if (localStorage.getItem("abp_jsonEditor")) {
        setTimeout(() => {
            jsonEditor.set(JSON.parse(localStorage.getItem("abp_jsonEditor")));
            $("#jse-bar-refresh").click();
        }, 200);
    }

    setInterval(() => {
        if (localStorage.getItem("abp_jsonEditor") !== jsonEditor.getText()) {
            localStorage.setItem("abp_jsonEditor", jsonEditor.getText());
        }
    }, 2000);
    ...
    if (localStorage.getItem("abp_entityInfo")) {
        $("#EntityInfo").val(localStorage.getItem("abp_entityInfo"));
        LoadTree();
    }

    $("#EntityInfo").keydown(function (event) {
        if (event.keyCode === 13) {
            event.preventDefault();
            localStorage.setItem("abp_entityInfo", event.currentTarget.value);
            toolMan.services.cSharp.cSharp.getAbpEntityInfo(event.currentTarget.value)
                .done((res) => {
                    var jsonVal = jsonEditor.get()
                    jsonVal["entityInfo"] = res;
                    jsonEditor.set(jsonVal);
                    abp.notify.success(l("The EntityInfo node has been updated"))
                })
        }
    });
